...
class StructuralCandidateRanker:
    ...
    def rank(
        ...
        if not legal_actions:
            legal_actions = list(self.action_generator.generate(hand, ctx))
        actions = [normalize_action(raw_action, i) for i, raw_action in enumerate(legal_actions)]
        has_normal_response = any(
            action.kind != "PASS" and not is_bomb_kind(action.kind)
            for action in actions
        )
        has_bomb_response = any(is_bomb_kind(action.kind) for action in actions)
        only_bomb_response = float(
            normalize_kind(ctx.current_kind) != "Lead"
            and has_bomb_response
            and not has_normal_response
        )
        ...
        native_super_universe = (
            self._native_super_hand_universe(
                tuple(hand),
                ctx,
                partition_context_suffix,
            )
            ...
            scored.append(
                ScoredAction(
                    ...
                    details={
                        ...
                        "raw_hand_count": best_partition.hand_count,
                        "effective_hand_count": best_partition.effective_hand_count,
        ...
        if normalize_kind(ctx.current_kind) == "Lead":
            ...
            scored = tactical_topk(scored, top_k)
        else:
            scored.sort(key=lambda row: row.score, reverse=True)
    ...
    def _prefilter_actions_approx(
        ...
        # The global stable score order restricted to one kind is exactly the
        # previous per-kind stable sort. Reuse it instead of sorting the same
        # actions once per kind and then sorting all actions again.
        ranked = sorted(
            actions,
        ...
        for same_kind in by_kind.values():
            ...
            for action in low_spend:
                add(action)
            low_rank = heapq.nsmallest(
                max(1, diverse_keep // 2),
                same_kind,
                key=lambda action: (
                    rank_score_cache[action.index],
                    spend_penalty_cache[action.index],
