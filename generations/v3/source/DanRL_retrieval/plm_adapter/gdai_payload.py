...
# Keep the vendored OpenGuanDan engine self-contained.  The optional Java bridge
# needs an external jar, so the retrieval service defaults to the pure Python
# rule implementation unless the caller deliberately overrides it.
os.environ.setdefault("GUANDAN_MOVE_ENGINE", "python")

from DanRL_retrieval.vendor.openguandan_engine import Moves
from DanRL_retrieval.vendor.openguandan_engine.python_rules import _plm_sequence_value
from DanRL_retrieval.vendor.openguandan_engine.python_rules import _plm_triple_plus_value
...
def latest_greater_action(payload: dict[str, Any]) -> list[Any] | None:
    ...
    for item in reversed(payload.get("play_history") or []):
        if str(item.get("action", "")).lower() != "play":
            continue
        decoded = decode_play_history_item(item, current_rank)
        if decoded is not None:
            return decoded
    return None


def decode_play_history_item(
    item: dict[str, Any], current_rank: str,
) -> list[Any] | None:
    """Decode one PLM public play event into canonical OpenGuanDan fields."""
...
def build_openguandan_action_list(payload: dict[str, Any]) -> list[Any]:
    current_rank = PLM_VALUE_TO_RANK.get(int(payload.get("level_value", 2)), "2")
