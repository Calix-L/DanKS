...
from .action_semantics import RANK_POINTS, can_beat, normalize_kind, plain_points
...
def _same_kind_control(group: CardGroup, ctx: RetrievalContext, target_kind: str) -> float:
    ...
    if kind in {"Single", "Pair", "Triple", "TriplePlus"}:
        ...
        base = plain_points(group.rank) / max(1, RANK_POINTS["A"])
...
def _current_trick_control(group: CardGroup, ctx: RetrievalContext) -> float:
    ...
    if not can_beat(group, target, ctx.cur_rank):
        return 0.0
...
class PartitionScorer:
    ...
    def _current_trick_control_cached(self, group: CardGroup, group_kind: str) -> float:
        ...
        if not can_beat(group, target, self.ctx.cur_rank):
            return 0.0
    ...
    def cover_score_entry(self, group: CardGroup, key: tuple | None = None) -> tuple[float, int, int, float, int, int, float, tuple[tuple[int, float], ...]]:
        ...
        if nonzero_count == 1:
            ...
            idx = -1
            control = 0.0
        return cached_value, group_single_count, group_low_count, group_debt, nonzero_count, idx, control, nonzero_controls
    ...
    def score_group_values(
        ...
        hand_count_score = -float(hand_count)
    ...
    def score_cover_values(
        ...
        group_entries: list[tuple[float, int, int, float, int, int, float, tuple[tuple[int, float], ...]]],
        primary_kind: str | None = None,
        pressure_values: tuple[float, ...] | None = None,
    ) -> tuple[float, float, float, float, float]:
        weights = self.weights
...
def _score_partition_uncached(
    ...
    hand_count_score = -float(partition.hand_count)
    card_value_score = 0.0
